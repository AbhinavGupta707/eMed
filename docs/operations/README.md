...
The public synthetic Vercel/Neon demo has observed live ElevenLabs and Fireworks paths. It remains a hackathon demo, not a production clinical environment. Physical iPhone/Safari and live VitalLens remain separate owner/device/provider gates.
...
| Public synthetic demo     | `APP_ENV=demo`, exact HTTPS `APP_BASE_URL`, `DATABASE_URL`, `DEMO_ACCESS_SECRET`, `FHIR_PROVIDER=fixture` | Frictionless bounded guest entry, signed `Secure; HttpOnly; SameSite=Strict` demo cookie, and PostgreSQL.                                                      | Browser-operable hackathon shape; guest entry is intentionally not real authentication.        |
...
| `NEXT_PUBLIC_VOICE_TEST_FIXTURE` | Omit normally. `synthetic` is accepted only by development/test browser builds and emits identifier-free in-memory transcript events; production builds reject it. | Public test control       |
...
- [ ] Patient and clinician links issue one-hour signed synthetic guest sessions without a visible code prompt; cookie, expiry, fixed role, rate-limit, and redirect-allowlist controls pass.
- [ ] Before any real-data or public-scale use, replace guest entry with real identity and add platform/WAF abuse controls.
