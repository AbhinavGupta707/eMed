...
| Protected synthetic demo  | `APP_ENV=demo`, exact HTTPS `APP_BASE_URL`, `DATABASE_URL`, `DEMO_ACCESS_SECRET`, `FHIR_PROVIDER=fixture` | Signed `Secure; HttpOnly; SameSite=Strict` demo cookie and PostgreSQL.                                                                                         | Intended hosted shape, but not yet browser-operable: this commit verifies cookies but has no login/session-issuance route. |
...
| `DATABASE_URL`                | PostgreSQL URL. Absence selects the in-memory demo fallback, so hosted promotion must prove the `postgres` runtime profile.                                      | Server-only secret        |
...
- [ ] A central owner has provided and reviewed a browser session-issuance flow. Signed-cookie verification alone is insufficient.
